...
for i, spec in enumerate(specifications):
    instance_type = tags['required']['instance']
    ...
    #instance['required_tages']["type"] = worker_type
...
# with open(config_path1 , 'r') as data_file:
#     updating_tags = data_file.read()
#     print(f"updating_tags : {updating_tags}")

# cluster_master_count = 0
# for instance in updating_tags["instances_list"]:
#     if instance.get("vm_worker_type") == "Cluster Master":
#         cluster_master_count +=1
#         if cluster_master_count == 2:
#             instance["vm_tags"]["type"] = "Standby Cluster Master"
# updated_json = json.dumps(str(updating_tags), indent=4)
# print(f"updated_json : {updated_json}")
# if not os.path.exists(os.path.dirname(config_path2)):
#     os.makedirs(os.path.dirname(config_path2))

# with open(config_path2, 'w') as tfvars_file:
#     tfvars_file.write(updated_json)

# print(f"Updated 'general_parameters' with new data in {config_path2}.")
